...
import { Plus, CreditCard, CheckCircle, XCircle, DollarSign, Calendar, TrendingUp } from 'lucide-react';
...
export function LoanManagement({ state, updateState }: LoanManagementProps) {
  ...
  const handleCreateLoan = () => {
    if (!selectedMemberId || !newLoan.amount || parseFloat(newLoan.amount) <= 0) {
    ...
    toast.success(`Loan of ${formatCurrency(amount)} created for member ${selectedMemberId}`);
  ...
  const handleLoanAction = (loanId: string, action: 'approve' | 'reject') => {
    ...
    const updatedNotifications = state.notifications.filter(n => 
      !(n.type === 'loan_application' && n.data?.loanId === loanId)
    );
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end space-x-2">
                <Button variant="outline" onClick={() => setCreateLoanOpen(false)}>
                  Cancel
                </Button>
              ...
              <TableBody>
                {pendingLoans.map((loan) => (
                  <TableRow key={loan.id}>
                    <TableCell>
                      <div>
                        <div className="font-medium">{getMemberName(loan.memberId)}</div>
