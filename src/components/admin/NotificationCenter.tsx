...
import { Bell, CheckCircle, XCircle, User, CreditCard, DollarSign, AlertCircle, UserPlus } from 'lucide-react';
...
export function NotificationCenter({ state, updateState }: NotificationCenterProps) {
  ...
  const handleNotificationAction = (notificationId: string, action: 'approve' | 'reject') => {
    ...
    if (notification.type === 'signup_request') {
      ...
      updatedUsers = state.users.map(user => {
        if (user.memberId === notification.memberId) {
          return {
            ...user,
            status: action === 'approve' ? 'active' as const : 'inactive' as const
          };
        }
        return user;
      });
      ...
      const loan = state.loans.find(l => l.memberId === notification.memberId && l.status === 'pending');
      if (loan) {
    ...
    const updatedNotifications = notifications.map(n => {
      if (n.id === notificationId) {
        return { ...n, status: action === 'approve' ? 'approved' as const : 'rejected' as const };
      }
      return n;
    });
    ...
    const member = state.users.find(u => u.memberId === notification.memberId);
    const actionText = notification.type === 'signup_request' ? 
      (action === 'approve' ? 'approved and activated' : 'rejected') :
      `${action}d`;
    toast.success(`${notification.type.replace('_', ' ')} ${actionText} for ${member?.name}`);
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage member requests and applications</p>
        </div>
      ...
      <div className="grid md:grid-cols-5 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {notifications.filter(n => n.type === 'loan_application').length}
            </div>
            <p className="text-xs text-muted-foreground">
              Loan requests
            </p>
                    ...
                    {notification.data && (
                      <div className="text-xs text-muted-foreground">
                        {notification.type === 'signup_request' && (
                          <div>
                            Email: {notification.data.email} | Mobile: {notification.data.mobile}
                          </div>
                        )}
                        {notification.type === 'profile_update' && (
                          <div>
                            New Mobile: {notification.data.mobile} | New Email: {notification.data.email}
                          </div>
                        )}
                        {notification.type === 'loan_application' && (
                          <div>
                            Amount: ${notification.data.amount?.toLocaleString()} | Term: {notification.data.termMonths} months
                          </div>
                        )}
                      </div>
                    )}
                ...
                .map((notification) => (
                  <div key={notification.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          <p className="font-medium text-sm">{notification.memberName}</p>
                          <Badge variant="outline" className="text-xs">
                            {notification.memberId}
                          </Badge>
                          <Badge variant="outline" className="text-xs">
